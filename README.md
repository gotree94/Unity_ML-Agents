...
> **Python 패키지**: ml-agents 1.1.0 (Python 3.10.1~3.10.12)  
...
| **Python** | Python 3.10.1 ~ 3.10.12 (3.10.12 권장) |
| **Conda** (선택) | Miniconda 3 (Python 3.10.1~3.10.12 포함) 또는 Anaconda 2024+ |
...
[Unity Editor (Unity 6000.0+)]
  ...
  └── GitHub 저장소 Project/ 폴더
        └── Assets/ML-Agents/Examples/ (예제 환경)
...
```bash
conda create -n mlagents_env python=3.10.12 -y
...
> **참고 (Unity 6000.0+):** ML-Agents 4.0부터 Package Manager의 **Samples 탭이 제거**되었습니다.
> 예제 환경은 GitHub 저장소의 `Project/` 폴더를 통해 제공됩니다.
> 두 가지 방법 중 하나를 선택하세요.

#### 방법 A: GitHub 저장소의 Project 폴더 열기 (권장)

이미 `git clone --branch release_23`을 완료했다면, 저장소 내의 `Project` 폴더를 Unity Hub에서 직접 엽니다.

1. Unity Hub 실행
2. **Add** → **Add project from disk** 클릭
3. 클론한 저장소의 `Project` 폴더 선택
   - 예: `C:\Users\Administrator\Desktop\ml-agents\Project`
4. **Open** 클릭
5. **Project** 창에서 `Assets/ML-Agents/Examples/` 경로에서 예제 확인
6. 원하는 예제 씬(예: `3DBall/Scenes/3DBall`) 열기

#### 방법 B: 기존 프로젝트에 Examples 폴더 복사

클론한 저장소에서 예제 폴더만 직접 복사할 수도 있습니다.

1. 클론한 저장소의 `Project/Assets/ML-Agents/Examples/` 폴더 통째로 복사
2. 본인의 Unity 프로젝트 `Assets/ML-Agents/Examples/` 경로에 붙여넣기
